...
public class Ejercicio2_Binario {
    public static Nodo insertar(Nodo raiz, int valor) {
        // Si llegamos a una posición nula, creamos y retornamos el nuevo nodo
        if (raiz == null) {
            return new Nodo(valor);
        }
        
        // Si el valor es menor, recursión hacia la izquierda
        if (valor < raiz.valor) {
            raiz.izquierdo = insertar(raiz.izquierdo, valor);
        } 
        // Si el valor es mayor o igual, recursión hacia la derecha
